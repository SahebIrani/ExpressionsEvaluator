...
public class Parser
{
    private readonly ArrayList FunctionList = new(new string[] { "abs", "acos", "asin", "atan", "ceil", "cos", "cosh", "exp", "floor", "ln", "log", "sign", "sin", "sinh", "sqrt", "tan", "tanh" });
    private double Factor;
    private Mode mode;

    // Constructor
    public Parser() => Mode = Mode.RAD;
    public Parser(Mode mode) => Mode = mode;
    ...
    public bool Evaluate(string Expression)
    {
        try
        {
            ...
            //             |   |     |
            //
            Regex regEx = new(@"(?<=[\d\)])(?=[a-df-z\(])|(?<=pi)(?=[^\+\-\*\/\\^!)])|(?<=\))(?=\d)|(?<=[^\/\*\+\-])(?=exp)", RegexOptions.IgnoreCase);
            ...
            // |_________|
            //
            this.Result = Convert.ToDouble(this.Solve(Expression));
            return true;
        }
        catch
        {
    ...
    {
        // Solve Sin, Cos, Tan...
        Regex regEx = new(@"([a-z]{2,})([\+-]?\d+,*\d*[eE][\+-]*\d+|[\+-]?\d+,*\d*)", RegexOptions.IgnoreCase);
        Match m = regEx.Match(Expression);
